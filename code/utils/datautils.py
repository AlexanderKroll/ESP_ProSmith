...
class SMILESProteinDataset(Dataset):
    ...
    def _load_protein_repr(self, protein_repr_path):
        map_loc = self.device #self.gpu if is_cuda(self.device) else self.device
        #print("Loading protein repr from: ", protein_repr_path)
    ...
    def update_subset(self):
        ...
        help_df["enzyme"] = [seq[:1018] for seq in help_df["enzyme"]]
        help_df = help_df.loc[help_df["enzyme"].isin(all_subset_sequences)]

        if self.train:
            help_df = help_df.sample(frac=1, random_state = self.random_state)
        help_df = help_df.reset_index(drop=True)

        #print(f"SMILES subset: {self.smiles_subset_no-1}, Protein Subset: {self.protein_subset_no}, Length help_df: {len(help_df)}")
    ...
    def __getitem__(self, idx):
        ...
        #print(f"Item: {idx}, len(help_df): {len(self.mappings)}, prev_subset_max_idx : {prev_subset_max_idx}, curr_subset_max_idx : {curr_subset_max_idx}")
        

        if idx >= len(self.mappings):
            #print(f"updating subset {self.subset_no}")
            self.update_subset()
        ...
        label, protein, smiles, index = float(0.0), self.mappings["enzyme"][idx], self.mappings["SMILES"][idx], int(self.mappings["index"][idx])
